...
class ScalpelSubject:
    ...
    @cached_property
    def gyrif_v(self):
        """Gyral hull surface vertices (``?h.pial-outer-smoothed``).

        This surface shares the pial surface's coordinate frame, so it is the
        correct reference for pial-to-hull sulcal depth. (The ``inflated``
        surface previously loaded here lives in a different frame, making the
        depth distances meaningless.) Generated by FreeSurfer's
        ``mris_fill``/``mris_smooth`` step; a clear FileNotFoundError is raised
        if it is absent.
        """
    ...
    def cortex_vertices(self):
        """Vertex indices of the cortex label (``?h.cortex.label``).

        Excludes the medial wall. FreeSurfer's mris_anatomical_stats masks the
        whole-hemisphere gray matter volume to this label; used for the same
        purpose here. A missing file raises a clear FileNotFoundError.
        """
    ...
    def gaussian_curvature(self):
        """Gaussian curvature values from FreeSurfer ``?h.curv.K``.

        Not produced by ``recon-all`` by default -- generate it with
        ``mris_curvature -k``. A missing file raises (via ``_load_curv_file``)
        rather than returning zeros, so Gaussian-derived statistics (intrinsic
        curvature index, folding index) cannot silently report 0 as if valid.
        """
        return self._load_curv_file('curv.K')

    @cached_property
    def k1_curvature(self):
        """First (max) principal curvature from FreeSurfer ``?h.white.max``.

        Generated by ``mris_curvature -max ?h.white``. Read directly rather than
        reconstructed from mean/Gaussian curvature so the folding index matches
        mris_anatomical_stats (validated on fsaverage lh: 459.0 vs 459).
        """
        return self._load_curv_file('white.max')

    @cached_property
    def k2_curvature(self):
        """Second (min) principal curvature from FreeSurfer ``?h.white.min``.

        Generated by ``mris_curvature -min ?h.white``.
        """
